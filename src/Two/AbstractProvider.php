...
namespace Lijinhua\HyperfSocialite\Two;
...
abstract class AbstractProvider implements ProviderContract
{
    ...
    public function redirect()
    {
        ...
        if ($this->usesPKCE()) {
            $this->session()->put('code_verifier', $this->getCodeVerifier());
        }

        return \Hyperf\Support\make(ResponseInterface::class)->redirect($this->getAuthUrl($state));
    }
    ...
    /**
     * @return \Lijinhua\HyperfSocialite\Two\User
    ...
    protected function getHttpClient()
    {
        if (is_null($this->httpClient)) {
            $this->httpClient = \Hyperf\Support\make(ClientFactory::class)->create();
        }
